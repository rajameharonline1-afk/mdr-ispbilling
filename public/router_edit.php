...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name && $type && $ip && $username && $password) {
        $stmt = db()->prepare("UPDATE routers SET 
            name = ?, type = ?, ip = ?, username = ?, password = ?, 
            api_port = ?, snmp_community = ?, status = ? WHERE id = ?");
        $stmt->execute([$name, $type, $ip, $username, $password, $api_port, $snmp_community, $status, $id]);
...
<?php if (hasPermission('edit.routers')){ ?>   
    ...
    <form method="POST" class="p-3 border rounded bg-light">
        ...
        <div class="mb-3">
            ...
            <input type="text" name="ip" class="form-control" value="<?php echo $router['ip']; ?>" required>
        </div>
        <div class="mb-3">
