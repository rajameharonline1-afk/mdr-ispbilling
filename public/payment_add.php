...
function ensure_default_account_id(PDO $pdo, int $received_by): int {
  // বাংলা: account_id না দিলে প্রথম active account নেবে; কোনো account না থাকলে default "Cash" বানাবে।
  if (!tbl_exists($pdo, 'accounts')) return 0;

  // 1) current user mapped to accounts.user_id
  if ($received_by > 0 && col_exists($pdo, 'accounts', 'user_id')) {
    $m = $pdo->prepare("SELECT id FROM accounts WHERE user_id=? LIMIT 1");
    $m->execute([$received_by]);
    $id = (int)($m->fetchColumn() ?: 0);
    if ($id > 0) return $id;
  }

  // 2) any active account
  if (col_exists($pdo, 'accounts', 'is_active')) {
    $m = $pdo->query("SELECT id FROM accounts WHERE is_active=1 ORDER BY id ASC LIMIT 1");
  } else {
    $m = $pdo->query("SELECT id FROM accounts ORDER BY id ASC LIMIT 1");
  }
  $id = (int)($m->fetchColumn() ?: 0);
  if ($id > 0) return $id;
  ...
  if (col_exists($pdo, 'accounts', 'type')) { $cols[] = 'type'; $vals[] = ':type'; $params[':type'] = 'cash'; }
  if (col_exists($pdo, 'accounts', 'number')) { $cols[] = 'number'; $vals[] = ':number'; $params[':number'] = null; }
  if (col_exists($pdo, 'accounts', 'is_active')) { $cols[] = 'is_active'; $vals[] = ':is_active'; $params[':is_active'] = 1; }
  // user_id optional; keep NULL to make it global
  if (col_exists($pdo, 'accounts', 'balance')) { $cols[] = 'balance'; $vals[] = ':balance'; $params[':balance'] = 0; }
  if (col_exists($pdo, 'accounts', 'created_at')) { $cols[] = 'created_at'; $vals[] = 'NOW()'; }

  $sql = "INSERT INTO accounts (".implode(',', array_map(fn($c)=>"`$c`",$cols)).") VALUES (".implode(',', $vals).")";
...
function is_duplicate_method_txn(PDO $pdo, string $method, string $txn_id): bool {
  $method = trim($method);
  $txn_id = trim($txn_id);
  if ($method === '' || $txn_id === '') return false;
  if (!tbl_exists($pdo, 'payments') || !col_exists($pdo, 'payments', 'method') || !col_exists($pdo, 'payments', 'txn_id')) return false;
  $st = $pdo->prepare("SELECT id FROM payments WHERE method=? AND txn_id=? LIMIT 1");
  $st->execute([$method, $txn_id]);
...
// ajax?
// বাংলা: কিছু UI (মডাল/পপআপ) ajax=1 না পাঠালেও XHR/fetch দিয়ে কল করে—সেক্ষেত্রে JSON রেসপন্স দিতে হবে।
$is_ajax = isset($_POST['ajax']) || isset($_GET['ajax']) || (($_POST['ajax'] ?? ($_GET['ajax'] ?? '')) === '1');
if (!$is_ajax) {
  $xrw = strtolower((string)($_SERVER['HTTP_X_REQUESTED_WITH'] ?? ''));
  if ($xrw === 'xmlhttprequest') $is_ajax = true;
}
if (!$is_ajax) {
  $accept = strtolower((string)($_SERVER['HTTP_ACCEPT'] ?? ''));
  if (str_contains($accept, 'application/json')) $is_ajax = true;
}
if (!$is_ajax && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && $is_popup) {
  $is_ajax = true;
}
if (!$is_ajax && ($_SERVER['REQUEST_METHOD'] ?? '') === 'POST') {
  // বাংলা: ব্রাউজার fetch/XHR সাধারণত Sec-Fetch-Mode: cors এবং Dest: empty পাঠায়;
  // নেভিগেশন/form submit হলে Mode: navigate, Dest: document হয়।
  $sfm = strtolower((string)($_SERVER['HTTP_SEC_FETCH_MODE'] ?? ''));
  $sfd = strtolower((string)($_SERVER['HTTP_SEC_FETCH_DEST'] ?? ''));
  if (($sfm !== '' && $sfm !== 'navigate') || ($sfd !== '' && $sfd !== 'document')) {
    $is_ajax = true;
  }
}
...
if ($purpose === 'advance') {
  ...
  if ($account_id <= 0) {
    $account_id = ensure_default_account_id($pdo, $received_by);
  }
  if ($account_id <= 0) {
  ...
  // বাংলা: বর্তমান স্কিমায় payments.invoice_id & payments.bill_id সাধারণত NOT NULL,
  ...
  if (col_exists($pdo,'payments','payment_date')){ $cols[]='payment_date'; $vals[]=':payment_date'; $params[':payment_date']=$paid_at; }
...
$hasInvoiceId = col_exists($pdo,'payments','invoice_id');
$hasBillId    = col_exists($pdo,'payments','bill_id');
if (!$hasInvoiceId && !$hasBillId){ rollback_if($pdo); jexit(['ok'=>false,'error'=>'schema_missing','message'=>'payments needs invoice_id or bill_id']); }

$cols = ['client_id','amount'];
$vals = [':client_id',':amount'];
$params = [ ':client_id'=>$client_id, ':amount'=>round($amount,2) ];

// বাংলা: এই স্কিমায় invoice_id এবং bill_id দুটোই আছে ও NOT NULL—দুটোই সেট করি।
if ($hasInvoiceId) { $cols[]='invoice_id'; $vals[]=':invoice_id'; $params[':invoice_id']=$invoice_id; }
if ($hasBillId)    { $cols[]='bill_id';    $vals[]=':bill_id';    $params[':bill_id']=$invoice_id; }
...
$sql = "INSERT INTO payments (".implode(',',$cols).") VALUES (".implode(',',$vals).")";
// Duplicate guard for uniq_method_txn (method+txn_id)
if ($txn_id !== '' && is_duplicate_method_txn($pdo, (string)$method, (string)$txn_id)) {
  rollback_if($pdo);
  jexit(['ok'=>false,'error'=>'duplicate_txn','message'=>'এই Transaction ID আগেই ব্যবহার করা হয়েছে। অনুগ্রহ করে নতুন/সঠিক Transaction ID দিন।']);
}
try {
  $pdo->prepare($sql)->execute($params);
} catch (PDOException $e) {
  // Catch duplicate unique key and return JSON instead of fatal HTML
  if (($e->getCode() === '23000') && str_contains($e->getMessage(), 'uniq_method_txn')) {
    rollback_if($pdo);
    jexit(['ok'=>false,'error'=>'duplicate_txn','message'=>'এই Transaction ID আগেই ব্যবহার করা হয়েছে। অনুগ্রহ করে নতুন/সঠিক Transaction ID দিন।']);
  }
  throw $e;
