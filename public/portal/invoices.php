...
<body>
          ...
          <div class="table-responsive">
            <table class="table table-sm table-hover align-middle mb-0">
              ...
              <?php else: foreach($rows as $r):
                  ...
                  // bKash payment options page (PGW highlighted) — due থাকলে enable
                  $bkashUrl = '/public/portal/payments.php?mode=pgw&ref='.urlencode($inv_no).'&amount='.urlencode(number_format($due,2,'.',''));
                ?>
                <tr>
                  ...
                  <td class="text-end">
                    <div class="btn-group btn-group-sm">
                      ...
                      <?php
                        $isDisabled = ($due <= 0);
                        $payHref = $isDisabled ? '#' : $bkashUrl;
                      ?>
                      <a
                        href="<?php echo h($payHref); ?>"
                        class="btn btn-outline-danger js-bkash-pay <?php echo ($isDisabled?'disabled':''); ?>"
                        data-ref="<?php echo h($inv_no); ?>"
                        data-amount="<?php echo h(number_format($due,2,'.','')); ?>"
                        <?php echo ($isDisabled?'aria-disabled="true" tabindex="-1"':''); ?>
                      >
  ...
  <!-- bKash Pay popup card (top-right) -->
  <style>
    #bkashPayCard{
      position: fixed;
      top: 86px;
  ...
  <div id="bkashPayCard" class="card shadow border-0">
    <div class="card-body">
      <div class="d-flex align-items-start gap-2">
        <div class="rounded-3 d-flex align-items-center justify-content-center"
             style="width:40px;height:40px;background:#fee2e2;color:#b91c1c;flex:0 0 auto;">
          <i class="bi bi-wallet2"></i>
        </div>
        <div class="flex-grow-1">
          <div class="fw-semibold">bKash পেমেন্ট</div>
          <div class="text-muted small">রেফারেন্স: <span id="bkashPayRef" class="fw-semibold"></span></div>
          <div class="text-muted small">Amount: <span id="bkashPayAmt" class="fw-semibold"></span> BDT</div>
        </div>
        <button type="button" class="btn btn-sm btn-outline-secondary" id="bkashPayClose" aria-label="Close">
          <i class="bi bi-x-lg"></i>
        </button>
      </div>
      <div class="d-flex gap-2 mt-3">
        <button type="button" class="btn btn-danger flex-grow-1" id="bkashPayGo">
          <i class="bi bi-box-arrow-up-right"></i> bKash পেজ ওপেন করুন
        </button>
        <button type="button" class="btn btn-outline-secondary" id="bkashPayCancel">Cancel</button>
      </div>
      <div class="form-text mt-2">এই বাটনে ক্লিক করলে একই ট্যাবে bKash পেমেন্ট পেজ ওপেন হবে।</div>
    </div>
  </div>
  ...
  <script>
    (function(){
      const card = document.getElementById('bkashPayCard');
      const refEl = document.getElementById('bkashPayRef');
      const amtEl = document.getElementById('bkashPayAmt');
      const btnGo = document.getElementById('bkashPayGo');
      const btnClose = document.getElementById('bkashPayClose');
      const btnCancel = document.getElementById('bkashPayCancel');

      if (!card) return;
      let currentUrl = '';
      let hideTimer = null;
      const setGoEnabled = (enabled) => {
        if (!btnGo) return;
        btnGo.disabled = !enabled;
        btnGo.classList.toggle('disabled', !enabled);
        btnGo.classList.toggle('btn-danger', !!enabled);
        btnGo.classList.toggle('btn-outline-secondary', !enabled);
        btnGo.innerHTML = enabled
          ? '<i class="bi bi-box-arrow-up-right"></i> bKash পেজ ওপেন করুন'
          : '<i class="bi bi-check2-circle"></i> কোনো বকেয়া নেই';
      };
      const show = () => {
      ...
      card.addEventListener('mouseleave', () => {
        clearTimeout(hideTimer);
        hideTimer = setTimeout(() => card.classList.remove('show'), 1500);
      });
      btnClose?.addEventListener('click', hide);
      btnCancel?.addEventListener('click', hide);
      btnGo?.addEventListener('click', () => {
        if (currentUrl) window.location.href = currentUrl; // same tab
      });

      document.addEventListener('click', (e) => {
        const btn = e.target.closest?.('.js-bkash-pay');
        if (!btn) return;
        e.preventDefault();
        const isDisabled = btn.classList.contains('disabled') || btn.getAttribute('aria-disabled') === 'true';
        currentUrl = isDisabled ? '' : (btn.getAttribute('href') || '');
        refEl.textContent = btn.getAttribute('data-ref') || '';
        amtEl.textContent = btn.getAttribute('data-amount') || '';
        setGoEnabled(!isDisabled);
        show();
      });
    })();
  </script>
