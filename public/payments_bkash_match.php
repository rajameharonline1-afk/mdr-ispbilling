...
function settle_payment(PDO $pdo, int $client_id, float $amount, string $trx_id, string $method, string $note, ?string $paid_at): array {
  ...
  // Sort by billing_month/invoice_date when available
  $orderCol = null;
  ...
  $sql = "SELECT i.id, i.payable, COALESCE(i.paid_amount,0) AS paid_amount
  ...
  if (!$invs) return ['applied_amount'=>0.0,'applied_invoices'=>[],'remaining'=>$amount,'error'=>'কোনো বকেয়া ইনভয়েস পাওয়া যায়নি।'];
  ...
  foreach ($invs as $inv) {
    ...
    $pdo->prepare("INSERT INTO payments (".implode(',', $fields).") VALUES (".implode(',', $placeholders).")")->execute($values);

    $newPaid = (float)$inv['paid_amount'] + $pay;
    $newStatus = null;
    if (in_array('status', $invCols, true)) $newStatus = ($newPaid >= (float)$inv['payable']) ? 'paid' : 'partial';
...
/* dup guard */
$dupSql = "SELECT id FROM payments WHERE method='bkash' AND (txn_id=? ";
$params = [$trxId];
if (col_exists($pdo,'payments','transaction_id')) { $dupSql .= " OR transaction_id=?"; $params[] = $trxId; }
$dupSql .= ") LIMIT 1";
...
if (($res['applied_amount'] ?? 0) <= 0) {
  $pdo->rollBack();
  if ($markProc) {
    $pdo->prepare("UPDATE sms_inbox SET error_msg=? WHERE id=?")->execute([$res['error'] ?? 'কোনো বকেয়া ইনভয়েস পাওয়া যায়নি।', $inbox_id]);
  }
  header("Location: /public/payments_bkash_inbox.php?status=unprocessed&msg=".urlencode('No due invoice found; nothing applied.'));
  exit;
}
