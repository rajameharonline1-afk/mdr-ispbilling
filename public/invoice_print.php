...
$__is_admin  = !empty($_SESSION['user']) || !empty($_SESSION['user_id']);
$__is_portal = !empty($_SESSION['portal_client_id']);
if ($__is_admin) {
...
$sel = "SELECT i.id, i.$amt_col AS amount, ".
       ...
       "WHERE i.id = ?".
       ($__is_portal ? " AND i.client_id = ?" : "");
$sti = $pdo->prepare($sel);
$sti->execute($__is_portal ? [$id, (int)($_SESSION['portal_client_id'] ?? 0)] : [$id]);
