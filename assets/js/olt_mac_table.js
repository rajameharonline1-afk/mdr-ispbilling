...
(function(){
  ...
  // Filter/search AJAX handler and modal binding moved from inline template
  (function(){
    ...
    function setTable(html) {
      ...
      const evt = new CustomEvent('olt-table-updated', { detail: { root: tableArea } });
    ...
    function syncPonOptions(options, current, clearSelection = false) {
      if (!ponSelect) return;
      const optsRaw = Array.isArray(options) ? options : [];
      const opts = optsRaw.map((item) => {
        if (item && typeof item === 'object') {
          const slot = Number(item.slot ?? item.value ?? 0);
          const label = String(item.label ?? ('EPONO/' + slot));
          return { slot, label };
        }
        const slot = Number(item || 0);
        return { slot, label: 'EPONO/' + slot };
      ...
      const slotValues = opts.map((item) => String(item.slot));
      opts.forEach(item => {
        const v = String(item.slot);
      ...
      if (opts.length === 0) {
        ...
        if (!clearSelection && slotValues.includes(String(prev))) {
          ponSelect.value = prev;
        } else if (current && slotValues.includes(String(current))) {
  ...
  // Config modal save/toast handlers
  (function(){
    const ensureHolder = () => {
    ...
    const rowKeyEl = document.getElementById('cfgRowKey');
    const oltIdEl = document.getElementById('cfgOltId');
    const portEl = document.getElementById('cfgPort');
    const onuEl = document.getElementById('cfgOnu');
    const sourceEl = document.getElementById('cfgSource');
    ...
    const bindConfigButtons = (root = document) => {
      root.querySelectorAll('.btn-config').forEach(btn => {
        btn.addEventListener('click', () => {
          if (!rowIdEl || !descEl) return;
          rowIdEl.value = btn.getAttribute('data-row-id') || '';
          if (rowKeyEl) rowKeyEl.value = btn.getAttribute('data-row-key') || '';
          if (oltIdEl) oltIdEl.value = btn.getAttribute('data-olt-id') || '';
          if (portEl) portEl.value = btn.getAttribute('data-port') || '';
          if (onuEl) onuEl.value = btn.getAttribute('data-onu') || '';
          if (sourceEl) sourceEl.value = btn.getAttribute('data-source') || '';
    ...
    // initial bind
    bindConfigButtons();
    // re-bind after AJAX table reloads
    document.addEventListener('olt-table-updated', (e) => {
      const targetRoot = e.detail?.root || document;
      bindConfigButtons(targetRoot);
    });
    if (modalForm) {
      modalForm.addEventListener('submit', async (e) => {
        ...
        try {
          ...
          if (json && json.ok) {
            ...
            const rowKey = (json && json.row_key) ? String(json.row_key) : (rowKeyEl ? String(rowKeyEl.value || '') : '');
            if (rowKey) {
              const keyCell = Array.from(document.querySelectorAll('.desc-text[data-row-key]')).find((el) =>
                (el.getAttribute('data-row-key') || '') === rowKey
              );
              if (keyCell) keyCell.textContent = descEl ? (descEl.value || '—') : keyCell.textContent;
            } else if (rid) {
