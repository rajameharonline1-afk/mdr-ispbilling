<?php
// /cron/enqueue_due_notifications.php
// বাংলা: বকেয়া ক্লায়েন্টদের জন্য নোটিফিকেশন কিউতে তোলা (SMS + Email)
...
// ===== shared helpers (client_is_due safe)
...
$idcol = col_exists($pdo,$CT,'id')?'id':(col_exists($pdo,$CT,'client_id')?'client_id':'id');
$namecol = col_exists($pdo,$CT,'name')?'name':(col_exists($pdo,$CT,'client_name')?'client_name':$idcol);
...
foreach ($rows as $r) {
  ...
  // বাংলা: payment URL/portal URL — তোমাদের অ্যাপের রুট অনুযায়ী অ্যাডজাস্ট করো
  $payload = [
    'amount'     => '', // চাইলে SUM due হিসাব করে দাও
    'invoice_id' => '',
