...
try{
  ...
  $opt_save_password=!empty($options['save_password'] ?? $options['import_password'] ?? false);
  $opt_import_name=!array_key_exists('import_name',$options) || !empty($options['import_name']);
  ...
  $profile_col = null;
  foreach (['ppp_profile','profile','profile_name','ppp_profile_name'] as $cand) {
    if (col_exists($pdo, 'clients', $cand)) { $profile_col = $cand; break; }
  }
  ...
  foreach($rows as $r){
    ...
    if(isset($cliMap[$pppoe_id])){
      ...
      $fields = ['router_id'=>$router_id,'updated_at'=>$now];
