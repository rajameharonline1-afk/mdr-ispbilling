...
try{
  ...
  if ($hasSpeed){ $cols[]='speed'; $vals[]='?'; }
  if ($hasValidity){ $cols[]='validity'; $vals[]='?'; }
  if ($hasCreated){ $cols[]='created_at'; $vals[]='NOW()'; }
  if ($hasUpdated){ $cols[]='updated_at'; $vals[]='NOW()'; }
  $sqlIns = "INSERT INTO packages (" . implode(',', $cols) . ") VALUES (" . implode(',', $vals) . ")";
  $ins = $pdo->prepare($sqlIns);
  foreach ($items as $it){
    if ($it['exists']) { $skipped++; continue; }
    $params = [$it['name'], $default_price];
    if ($hasSpeed){ $params[] = 'N/A'; }
    if ($hasValidity){ $params[] = 30; }
    $ins->execute($params);
    $created++;
  }
