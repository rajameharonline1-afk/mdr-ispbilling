...
require_once __DIR__ . '/../location_options.php';
...
@include_once __DIR__ . '/../audit.php'; // (বাংলা) থাকলে অডিট লগ করবো
...
function audit_client(string $action, int $client_id, array $details = []): void {
    ...
    try {
        // (বাংলা) call_user_func দিয়ে স্ট্যাটিক অ্যানালাইসিস টাইপ ওয়ার্নিং এড়াই
        call_user_func('audit_log', 'client', $client_id, $action, $details);
...
function save_photo_with_pppoe_filename(string $pppoe_id): array {
    ...
    $mime = function_exists('finfo_open') ? (function($tmp){
        ...
        $m  = finfo_file($fi, $tmp);
        // finfo_close deprecated; letting $fi go out of scope is fine
        return $m;
...
$page_title = $page_title ?? "Add Client";
