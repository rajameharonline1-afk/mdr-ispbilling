...
// Backwards-compatible helper: validate an explicit token string.
// Some pages use csrf_validate($_POST['csrf']).
if (!function_exists('csrf_validate')) {
  function csrf_validate(string $token): bool {
    $stored = csrf_session_token();
    return ($token !== '' && $stored && hash_equals((string)$stored, (string)$token));
  }
}
